...
### Phase 2 — Data integrity & Firestore cost (blocks release)

Four defects found in the Firestore data-layer audit: non-atomic stock writes,
sequence numbers generated by counting collections, unbounded dashboard listeners,
and factory-wide reads in the customer ledger. The last two are why the daily
free-tier quota is exceeded. Detailed in:

- [`MFMS_Phase2_Data_Integrity_Cost_Sprints.md`](./MFMS_Phase2_Data_Integrity_Cost_Sprints.md) — **S36–S41** (~3 weeks)

No new features; two of the four are silent data-corruption bugs, so this should
land before any further feature work.
